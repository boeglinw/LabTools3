...
class histo:
    ...
    def __init__(self,\
        ...
        if values != None:
            ...
            if (range == None) and (bins == None):
                self.fill(values, **kwargs)
            elif (range != None) and (bins == None):
            ...
            self.res = histogram
        elif (bin_center != None) and (bin_content != None):
    ...
    def sum(self, xmin = None, xmax = None):
        ...
        if (xmin == None) and (xmax == None):
            ...
            imax = self.find_bin(xmax)
            if (imax == None):
                raise histoError('invalid xmax value = ', xmax)
            sum = (self.bin_content[:imax+1]).sum()
            sum_err = np.sqrt( (self.bin_error[:imax+1]**2).sum())
        elif (xmax == None):
            imin = self.find_bin(xmin)
            if (imin == None):
                raise histoError('invalid xmin value = ', xmin)
            sum = (self.bin_content[imin:]).sum()
            sum_err = np.sqrt( (self.bin_error[imin:]**2).sum())
        else:
            imax = self.find_bin(xmax)
            imin = self.find_bin(xmin)
            if (imax == None):
            ...
            if (imin == None):
                raise histoError('invalid xmin value = ', xmin)
            sum = (self.bin_content[imin:imax+1]).sum()
            sum_err = np.sqrt( (self.bin_error[imin:imax+1]**2).sum())
        return (sum, sum_err)
    ...
    def fit(self, xmin = None, xmax = None):
        """
        ...
        if (xmin == None) and (xmax == None):
            ...
            if self.set_window:
                ...
                imin = self.find_bin(self.win_min)
                if (imin == None):
                    raise histoError('invalid win_min value = ', self.win_min)
                imax = self.find_bin(self.win_max)
                if (imax == None):
                    raise histoError('invalid win_max value = ', self.win_max)
                self.fit_indx = list(range(imin, imax))
            else:
                # if not use all data
                self.fit_indx = list(range( len(self.bin_center)))
        elif (xmin == None):
            imax = self.find_bin(xmax)
            if (imax == None):
                raise histoError('invalid xmax value = ', xmax)
            if self.set_window:
                ...
                imax_w = self.find_bin(self.win_max)
                if (imax_w == None):
                    raise histoError('invalid win_max value = ', self.win_max)
                self.fit_indx = list(range( min(imax, imax_w)))
            else:
                self.fit_indx = list(range( imax))
        elif (xmax == None):
            imin = self.find_bin(xmin)
            if (imin == None):
                raise histoError('invalid xmin value = ', xmin)
            if self.set_window:
                ...
                imin_w = self.find_bin(self.win_mmin)
                if (imin_w == None):
                    raise histoError('invalid win_min value = ', self.win_min)
                self.fit_indx = list(range( max(imin, imin_w), len(self.bin_center)))
            else:
                self.fit_indx = list(range( imin, len(self.bin_center)))
        else:
            imax = self.find_bin(xmax)
            imin = self.find_bin(xmin)
            if (imax == None):
            ...
            if self.set_window:
                ...
                imin_w = self.find_bin(self.win_min)
                if (imin_w == None):
                    raise histoError('invalid win_min value = ', self.win_min)
                imax_w = self.find_bin(self.win_max)
                if (imax_w == None):
                ...
                self.fit_indx = list(range( max(imin, imin_w), min(imax, imax_w)))
            else:
                self.fit_indx = list(range(imin, imax))
    ...
    def calc_fit_plot(self):
        # plot the fit
        x = np.linspace(self.bins[self.fit_indx][0], self.bins[self.fit_indx][-1:][0], 100.)
        self.fit_dict['xpl'] = x
        self.fit_dict['ypl'] = self.fit_func(x)
